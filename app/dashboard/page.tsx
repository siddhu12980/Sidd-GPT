...
export default function TestPage() {
  ...
  const testRateLimiting = async () => {
    if (!user) {
      alert("Please log in to test rate limiting");
      return;
    }
  ...
  const fetchUsageData = async (period: string = "day") => {
    if (!user) {
      setUsageData({ error: "Please log in to view usage data" });
      return;
    }
        ...
        <div className="mb-6 p-4 bg-white rounded-lg shadow-md">
          ...
          {user ? (
            ...
            <div className="text-sm text-orange-600">
              <span className="px-2 py-1 bg-orange-100 rounded-full text-xs">
                Not Authenticated - Usage data will not be available
              </span>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-semibold mb-4">
              ...
              {user && (
                <span className="text-sm font-normal text-gray-500">
                  (as {user.primaryEmailAddress?.emailAddress || "current user"}
                  )
                </span>
              )}
            </h2>
            <p className="text-sm text-gray-600 mb-4">
              {user
                ? "This will send 15 rapid requests to test the rate limit for your account."
                : "Please log in to test rate limiting."}
            </p>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="flex gap-2 mb-4">
              <button
                ...
                disabled={!user}
                className="px-3 py-2 bg-purple-500 text-white rounded hover:bg-purple-600 disabled:opacity-50 disabled:cursor-not-allowed text-sm"
              >
                Today
              </button>
              <button
                onClick={() => fetchUsageData("week")}
                disabled={!user}
                className="px-3 py-2 bg-purple-500 text-white rounded hover:bg-purple-600 disabled:opacity-50 disabled:cursor-not-allowed text-sm"
              >
                This Week
              ...
              {!user && (
                <span className="px-3 py-2 bg-gray-300 text-gray-600 rounded text-sm">
                  Login Required
                </span>
              )}
            </div>
